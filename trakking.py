...
#Lucas Kanade tracer
def good_points_to_track(rect,gray):
...
def trekking(video_src):
    ...
    while True:
        ...
        if firstDetected:
            ...
            p[0], st, err = cv2.calcOpticalFlowPyrLK(old_gray, gray, p_old[0], None, **lk_params) 
            p[1], st, err = cv2.calcOpticalFlowPyrLK(old_gray, gray, p_old[1], None, **lk_params)
            #print(len(p[1])+len(p[0]))
            try:
                good_new1 = p[0][st==1]
                good_old1 = p_old[0][st==1]   
                good_new2 = p[1][st==1]
                good_old2 = p_old[1][st==1]
            except:
                detected = False
                firstDetected = False
